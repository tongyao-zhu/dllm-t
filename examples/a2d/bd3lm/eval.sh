...
if [ -z "$model_name_or_path" ]; then
    model_name_or_path="dllm-collection/Qwen3-0.6B-diffusion-bd3lm-v0.1"
    echo ">>> model_name_or_path is not provided, using default value"
    common_args="--model bd3lm --apply_chat_template"

else
    echo ">>> model_name_or_path is provided, using the provided value: ${model_name_or_path}"
    common_args="--model bd3lm "
